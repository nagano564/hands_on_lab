...
def sentiment_analysis():
    with st.container():
        ...
        if entered_text:
            ...
            cortex_response = session.sql(f"select AUTHOR_ID, review, snowflake.cortex.sentiment(review) as sentiment_score from app_reviews;").to_pandas()
            st.caption("Score is between -1 and 1; -1 = Most negative, 1 = Positive, 0 = Neutral")  
            st.write(cortex_response)

            response_for_graph = session.sql(f"SELECT AUTHOR_ID, AVG(snowflake.cortex.sentiment(review)) AS avg_sentiment FROM app_reviews GROUP BY AUTHOR_ID;").to_pandas()  
            ...
            fig = px.bar(response_for_graph, x="AUTHOR_ID", y="AVG_SENTIMENT", color='LEGEND',
                         labels={"AUTHOR_ID": "Author ID", "AVG_SENTIMENT": "Average Sentiment"},
...
def complete():
    with st.container():
        ...
        entered_text = st.text_area("Enter text",label_visibility="hidden",height=400,placeholder='For example: call review')    
        if entered_text:
            entered_text = entered_text.replace("'", "\\'")
            prompt = f"Summarize this review in less than 200 words. Put the sentiment as positive or negative and summary in json format. Keep the summary to 15 words or less. Only return the JSON object. Nothing after.: {entered_text}"
